...
source('~/repos/portfolio/demonstrative/R/pbto2/nonlinear_ord_utils.R')
...
d.model <- get.stan.data(d.stan, static.features, ts.feature, n.outcome=3)
setwd('~/repos/portfolio/demonstrative/R/pbto2/models/stan')
model.file <- 'nonlinear_ord.stan'

posterior <- stan(model.file, data = d.model,
                  warmup = 200, iter = 1000, thin = 3, 
...
pars <- c('beta', 'betaz', 'a1', 'a2', 'b1', 'b2', 'c', 'p', 'outcome_cutpoints')
post <- rstan::extract(posterior)
print(posterior, pars)

rstan::traceplot(posterior, c('beta', 'betaz', 'a1', 'a2', 'b1', 'b2', 'c', 'outcome_cutpoints'))
plot(posterior)
...
p2 <- post.summary %>% 
  filter(!variable %in% c('lower_center', 'upper_center', 'weight_magnitude')) %>%
