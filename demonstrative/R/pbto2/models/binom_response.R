...
d.model <- list(
  ...
  pbto2 = d.stan$pbto2,
  pbto2_cutpoints = scale.cutpoint(seq(5,25, length.out=n.cp))
)
...
model.file <- 'cutpoint_long_binom_marginal.stan'

# Non-parallel fitting
posterior <- stan(file = model.file, data = d.model, warmup = 250, iter = 500, chains = 1, 
...
beta.post$alpha    <- post$alpha %>% as.numeric
