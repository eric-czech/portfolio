...
save(fit, file='/Users/eczech/data/ptbo2/export/mcmc_fit_long_cutpoint_v1.Rdata')
...
beta.post %>% mutate(pbto2_cp=unscale(pbto2_cp, 'pbto2')) %>% 
  ggplot(aes(x=pbto2_cp)) + geom_histogram(binwidth=1, alpha=.5) + 
  theme_bw() + ggtitle('Pbto2 Cutpoint Estimates') + 
  xlab('Pbto2 Cutoff') + 
  ggsave('/Users/eczech/data/ptbo2/images/pbto2_cp1.png')

beta.post %>% filter(pbto2_cp > -1) %>% 
  ...
  ggsave('/Users/eczech/data/ptbo2/images/pbto2_coef.png')
...
beta.post %>% melt(id.vars='samp_id') %>% 
  ...
  ggsave('/Users/eczech/data/ptbo2/images/coefs1.png')
