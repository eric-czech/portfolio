...
m1 <- glmulti('gos', p, db, family='binomial', level=1, crit = "aic")
m2 <- glmulti('gos', p, db, family='binomial', level=1, crit = "aicc")
m3 <- glmulti('gos', p, db, family='binomial', level=1, crit = "bic")
summary(m3@objects[[1]])

glm.res.lvl.1 <- glmulti('gos', p, db, family='binomial', level=1, crit = "aic")
...
rf.res <- randomForest(factor(gos) ~ . - uid, data=db)
