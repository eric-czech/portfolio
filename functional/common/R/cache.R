...
Cache <- setRefClass("Cache",
  fields = list(dir = "character", project = "character"),
  methods = list(
    initialize = function(..., dir=file.path('~', '.Rcache'), project='default'){
        callSuper(..., dir=dir, project=project)
    },
    normalizeKey = function(key){ paste0(key, '.Rdata') },
    ...
    load = function(key, loader, compress=T){
    ...
    store = function(key, object, compress=T){
